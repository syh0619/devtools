  ...
  <div class="tool-page">
    ...
    <div class="container">
      <div class="input-section">
        <h3>输入{{ mode === 'encode' ? '文本' : 'Base64' }}</h3>
        <el-input
          v-model="inputText"
          type="textarea"
          :rows="15"
...
const process = async () => {
  if (!inputText.value.trim()) {
    ElMessage.warning(mode.value === 'encode' ? '请输入要编码的文本' : '请输入 Base64 字符串')
    return
  }
  
  try {
    const url = mode.value === 'encode' 
      ? 'http://localhost:8080/api/text/base64/encode'
      : 'http://localhost:8080/api/text/base64/decode'
    
    const param = mode.value === 'encode' ? { text: inputText.value } : { base64: inputText.value }
